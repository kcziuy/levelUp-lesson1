...
2. Скачиваем credentials
3. Устанавливаем aws CLI, конфигурируем с помощью aws configure (данные из credentials, регион, формат вывода)
4. Скачиваем terraform с оф сайта
  4.а для винды кидаем в любую удобную директорию, добавляем в path путь до диры
  4.б для линукса копируем в /usr/bin
5. Пишем код для развертывания:
  5.1 в provider.tf добавляем aws (так принято)
  5.2 в main.tf пишем код для генерации ssh-ключа любым алгоритмом любой длины (у меня RSA 4096)
  5.3 туда же пишем код для добавления ключа в наш проект
  5.4 дальше security groups - разрешаем 22 порт (ssh), 80 порт (http), 443 порт (https) (не обязательно), удаляем дефолтные настройки (0 порт)
  5.5 код для поиска нужной дистры (подглядеть можно в консоли aws на из сайте), либо добавляем просто его id
  5.6 код для развертывания, в конце развертывания не забываем providers который обновит инфу о репозитории (apt update) и поставит пакет nginx (apt install nginx) и запуска демона nginx (service start nginx)
6. Делаем terraform init (лучше сделать это после 5.1 - так будут хайлайты в коде)
7. Проверяем будущие изменения в terraform plan
8. Если все ок, делаем terraform apply, ждем много секунд
9. Когда все готово, делаем cat terraform.ftstate | grep "public_ip", заходим в браузере на этот ip
10. Та-дам, nginx приветствует вас
11. Сносим все нафиг командой terraform destroy
12. Перед тем, как все пушить в гит, убеждаемся, что в .gitignore указан ключ и tfstate, иначе злобные хацкеры заберут все ваше имущество и кота.
13. Пушим в гит
14. Пишем эту инструкцию.
15. ???
16. PROFIT!!!
